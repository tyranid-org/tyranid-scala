  ...
  override def draw( pScope:Scope ) =
    if ( inputOnly ) {
      ...
      <div id={ va.name + "_c" } class={ "fieldc" + ( invalid |* " invalid" ) }>
       ...
       <div class={ "inputc" + att.domain.inputcClasses + ( { create |* " withNew" } )}>{ att.domain.ui( scope, this ) }{ create |* <span class="createNew"> <a href="#" class="tip" tip="Create New">new</a></span> }</div>
