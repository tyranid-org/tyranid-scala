  ...
  def handle(web: WebContext) {
    ...
    case "/edit" =>
      ...
      val ui = user.view.ui(
         "editSms",
         Grid(
           Row( Field( 'sms_phone, Seq( "readonly" -> "1" ) ) ),
           Row( Field( 'sms_ok, Seq( "labels" -> "Validate Now|Invalidate and enter new number", "href" -> ( web.path + "?toggleSmsOk=1" ) ), uiStyle = Field.UI_STYLE_TOGGLE ) ),
