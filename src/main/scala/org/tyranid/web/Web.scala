...
object WebFilter {
  ...
  val assetPattern = java.util.regex.Pattern.compile( "((([^\\s]+(\\.(?i)(ico|jpeg|jpg|png|gif|bmp|js|css|ttf|eot|woff|svg|html|map|htc|vtt|odt|wav)))|.*robots\\.txt)$)|.*io/thumb.*|.*/sso/.*" )
